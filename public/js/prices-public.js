jQuery(document).ready(function ($) {
  ...
  languageStrings.yearly = $(
    '.hidden-translate[data-translateID="yearly"]'
  ).val();
  languageStrings.totalPrice = $(
    '.hidden-translate[data-translateID="totalPrice"]'
  ).val();
  languageStrings.discount = $(
    '.hidden-translate[data-translateID="discount"]'
  ).val();
  languageStrings.fullPrice = $(
    '.hidden-translate[data-translateID="fullPrice"]'
  ).val();
  languageStrings.addons = $(
    '.hidden-translate[data-translateID="addons"]'
  ).val();
  languageStrings.free = $('.hidden-translate[data-translateID="free"]').val();
  languageStrings.implementation = $(
    '.hidden-translate[data-translateID="Implementation"]'
  ).val();
  ...
  function showRecruit() {
    ...
    $('#productImageAndName').html(
      '<figure><img src="' +
        priceData.imageUrl +
        '" alt="product logo"/></figure> HR-ON Recruit'
    );
  ...
  function showStaff() {
    ...
    $('#productImageAndName').html(
      '<figure><img src="' +
        priceData.imageUrl +
        '" alt="product logo"/></figure> HR-ON Staff'
    );
  ...
  function showBoth() {
    ...
    $('#productImageAndName').html(
      '<figure><img src="' +
        priceData.imageUrl +
        '" alt="product logo"/></figure> HR-ON Suite'
    );
    $('#recruitmentsSelectContainer').addClass('active');
    $('#staffAmountContainer').addClass('active');
  ...
  function renderRecruitPrice() {
    ...
    var html = `
		<div class="each-basic-price">
			...
			<div class="summary-price">${languageStrings.from} ${numberToNiceString(
      priceData.jobAmount
    )}</div>
		</div>
		`;
  ...
  function renderSupport() {
    ...
    var html = `
		<div class="each-basic-price" >
			...
			<div class="summary-price">${languageStrings.free}</div>
		</div>
		<div class="each-basic-price">
			<div>Implementering</div>
			<div class="summary-price">${languageStrings.free}</div>
		</div>
		`;
    return html;
  ...
  function renderPrices() {
    ...
    if (currentProduct === 'recruit') {
      var fullPrice =
        parseInt(priceData.jobAmount) + parseInt(priceData.supportPrice);
    } else if (currentProduct === 'staff') {
      var fullPrice =
        parseInt(priceData.staffAmount) + parseInt(priceData.supportPrice);
    } else if (currentProduct === 'both') {
      var fullPrice =
        parseInt(priceData.jobAmount) +
        parseInt(priceData.staffAmount) +
        parseInt(priceData.supportPrice);
    } else {
      var fullPrice =
        parseInt(priceData.jobAmount) +
        parseInt(priceData.staffAmount) +
    ...
    if (priceData.discount >= 1) {
      ...
      $('#finalSummary').append(`
        ...
        } <span class="ba_product_price" /> ${numberToNiceString(
        totalPrice
      )}</span></div>
			</div>
			`);
  ...
  function numberToNiceString(nStr) {
    ...
    if (languge === 'danish') {
      ...
      return (
        '<span class="currency-display">kr.</span> <div class="priceForDisplay" data-price="' +
        nStr +
        '">' +
        x1 +
        x2 +
  ...
  $(document).on('click', '#komIGangButton', function (e) {
    ...
    setTimeout(function () {
      ...
      $('#productChoosenSubmit').html(
        `<figure><img src="${priceData.imageUrl}" alt="product logo" /></figure> ${productName}`
      );
      $('#checkoutContainer').removeClass('active');
      $('#checkoutContainer').removeClass('fade-out-bottom');
      $('#submitContainer').addClass('active');
